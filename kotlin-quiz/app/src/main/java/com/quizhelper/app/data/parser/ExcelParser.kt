...
/**
 * Parses Excel (.xlsx/.xls) files into Question objects.
 */
object ExcelParser {
    ...
    fun parse(context: Context, uri: Uri): ParseResult {
        ...
        try {
            ...
            for (row in sheet) {
                rows.add(row.map { cell -> cell.toString().trim() })
            }
            workbook.close()
            inputStream.close()
            ...
            for (i in (headerRowIndex + 1) until rows.size) {
                ...
                // Trim trailing empty options
                while (options.isNotEmpty() && options.last().isBlank()) {
    ...
    fun parseAnswer(raw: String, optionCount: Int): List<Int> {
        ...
        // Boolean: 正确/对/true -> 0, 错误/错/false -> 1
        ...
        // Continuous letters: "AB" "ABCD"
        if (Regex("^[A-Z]+$").matches(upper)) {
            return upper.map { it - 'A' }.filter { it in 0 until optionCount }
        }

        // Separated letters: "A,B", "A、B", "A B"
        ...
        // Strip bracket annotations and retry
        ...
        // Extract letter sequence from text
        val letterMatch = Regex("[A-Z]{1,6}([\\s,，、]+[A-Z])*", RegexOption.IGNORE_CASE).find(s)
        if (letterMatch != null) {
            return extractLetters(letterMatch.value, optionCount)
        }
    ...
    private fun getCol(map: ColumnMap, key: String): Int = when (key) {
        "type" -> map.type; "stem" -> map.stem
        "optionA" -> map.optionA; "optionB" -> map.optionB
        "optionC" -> map.optionC; "optionD" -> map.optionD
        "optionE" -> map.optionE; "optionF" -> map.optionF
        "answer" -> map.answer; "analysis" -> map.analysis
        else -> -1
    }

    private fun setCol(map: ColumnMap, key: String, value: Int): ColumnMap = when (key) {
        "type" -> map.copy(type = value)
        "stem" -> map.copy(stem = value)
        "optionA" -> map.copy(optionA = value)
        "optionB" -> map.copy(optionB = value)
        "optionC" -> map.copy(optionC = value)
        "optionD" -> map.copy(optionD = value)
        "optionE" -> map.copy(optionE = value)
        "optionF" -> map.copy(optionF = value)
        "answer" -> map.copy(answer = value)
        "analysis" -> map.copy(analysis = value)
        else -> map
    }
}
